...
import { detectSearchType, normalizeText, getSchoolTypeSynonyms, buildNumberTokenRegex } from "@/lib/search-utils"
...
export async function searchEstablecimientos(searchTerm: string): Promise<SearchResult[]> {
  ...
  if (searchType.type === "codigo_organismo") {
    ...
    if (error) {
      console.error("Error searching organismo by code:", error)
      return []
    }
  ...
  if (searchType.type === "cue") {
    ...
    if (error) {
      console.error("Error searching by CUE:", error)
      return []
    }
  ...
  if (searchType.type === "predio") {
    ...
    if (error) {
      console.error("Error searching by PREDIO:", error)
      return []
    }
  ...
  if (searchType.type === "school_level") {
    ...
    if (synonyms.length > 0) {
      ...
      if (error) {
        console.error("Error searching by school level:", error)
        return []
      }
  ...
  if (searchType.type === "school_number") {
    ...
    if (schoolType) {
      ...
      if (synonyms.length > 0) {
        ...
        if (typeError) {
          console.error("Error searching by school type:", typeError)
          return []
        }
    ...
    if (allError) {
      console.error("Error searching schools:", allError)
      return []
    }
