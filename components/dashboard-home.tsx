...
>
  ...
  <CardHeader className="pb-3">
    <CardTitle className="flex items-center justify-between gap-2 text-sm font-medium text-slate-600 dark:text-gray-100">
      ...
      <ChevronRight className="h-4 w-4 text-slate-400 dark:text-gray-300 transition-transform group-hover:translate-x-0.5 group-hover:text-[#6366f1]" />
    </CardTitle>
  </CardHeader>
  <CardContent>
    <p className="text-4xl font-bold text-[#6366f1]">{metrics.totalOrganismos.toLocaleString("es-AR")}</p>
    <p className="mt-1 text-xs text-slate-400 dark:text-gray-300">Ver listado completo</p>
  </CardContent>
</Card>
        ...
        <div className={`container mx-auto px-4 transition-all duration-500 ease-in-out ${hasSearched ? "py-3 sm:py-4" : "py-6 sm:py-8"}`}>
          <div
            className={`flex flex-wrap items-end justify-between gap-3 transition-all duration-500 ease-in-out overflow-hidden ${
              hasSearched ? "mb-2 max-h-0 opacity-0" : "mb-4 max-h-40 opacity-100"
            }`}
          >
            <div>
              <h2 className="text-xl font-semibold text-slate-800 dark:text-white sm:text-2xl">Buscador de establecimientos</h2>
              <p className="text-sm text-slate-600 dark:text-gray-100">Busca por CUE, PREDIO, tipo de escuela, o nombre</p>
      ...
      <div className="container mx-auto flex flex-1 flex-col gap-6 px-4 py-6 sm:py-8 lg:flex-row lg:items-start lg:gap-8">
        <aside
          className={`transition-all duration-500 ease-in-out lg:shrink-0 ${
            hasSearched
              ? "max-h-0 opacity-0 overflow-hidden lg:max-h-none lg:w-0 lg:-translate-x-4 lg:pointer-events-none"
              : "max-h-[1000px] opacity-100 lg:w-72 lg:translate-x-0"
          }`}
        >
            ...
            <Card className="relative overflow-hidden border-2 border-dashed border-slate-300/60 shadow-sm bg-slate-50/30 dark:border-white/20 dark:shadow-lg dark:bg-white/5 dark:backdrop-blur-sm">
              <CardContent className="flex min-h-[300px] items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="mt-2 text-sm text-slate-400 dark:text-gray-300">
            ...
            {metrics.fedBreakdown.map((item) => {
              ...
              const rowContent = (
                <>
                  ...
                  <span className="whitespace-nowrap text-right text-sm font-semibold text-slate-700 dark:text-gray-100">
      ...
      <Dialog open={openDialog === "organismos"} onOpenChange={(open) => !open && setOpenDialog(null)}>
        <DialogContent className="max-w-[calc(100%-1.5rem)] sm:max-w-4xl">
          ...
          <div className="max-h-[70vh] overflow-y-auto pr-1">
            {isLoadingOrganismos ? (
              <div className="flex min-h-[200px] items-center justify-center text-sm text-slate-400 dark:text-gray-300">
                Cargando organismos…
              </div>
            ) : (
