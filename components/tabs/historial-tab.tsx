...
export function HistorialTab({ establecimientoId }: { establecimientoId: string }) {
  ...
  if (entries === null) {
    return <div className="py-8 text-center text-sm text-slate-400">Cargando historial…</div>
  }
  ...
  if (entries.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-sm font-medium text-slate-300">Todavía no hay cambios registrados</p>
  ...
  return (
    <div className="space-y-4 py-4">
      {groups.map((group) => (
        <div key={group.key} className="rounded-lg border border-slate-700/50 bg-card p-4">
          <div className="mb-3 flex items-center justify-between">
            <span className="rounded-full bg-slate-800/60 px-2.5 py-0.5 text-xs font-semibold uppercase tracking-wide text-slate-300">
              {group.seccion}
            </span>
          ...
          <div className="space-y-2">
            {group.items.map((item) => (
              <div key={item.id} className="flex flex-wrap items-center gap-2 text-sm">
                <span className="font-medium text-slate-200">{item.campo}:</span>
                <span className="text-slate-400 line-through">{item.valor_anterior || "(vacío)"}</span>
                <ArrowRight className="h-3 w-3 text-slate-300" />
                <span className="text-slate-100">{item.valor_nuevo || "(vacío)"}</span>
              </div>
            ))}
          </div>
        </div>
