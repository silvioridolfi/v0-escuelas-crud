...
export function HistorialTab({ establecimientoId }: { establecimientoId: string }) {
  ...
  if (entries.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        <History className="mb-3 h-10 w-10 text-slate-300 dark:text-gray-500" />
        <p className="text-sm font-medium text-slate-600 dark:text-gray-200">Todavía no hay cambios registrados</p>
        <p className="mt-1 text-xs text-slate-400 dark:text-gray-400">Los cambios que se guarden de acá en más van a aparecer acá</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4 py-4">
      {groups.map((group) => (
        <div key={group.key} className="rounded-lg border border-slate-200 dark:border-border bg-white dark:bg-card p-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="text-xs text-slate-400 dark:text-gray-400">{formatFecha(group.created_at)}</span>
          </div>
          <div className="space-y-2">
            {group.items.map((item) => (
              <div key={item.id} className="flex flex-wrap items-center gap-2 text-sm">
                ...
                <span className="text-slate-400 dark:text-gray-400 line-through">{item.valor_anterior || "(vacío)"}</span>
                <ArrowRight className="h-3 w-3 text-slate-300 dark:text-gray-500" />
