/**
 * Returns a consistent color class for each FED
 * "Sin FED asignado" gets a warning color (yellow/amber)
 */
export function getFedBadgeColor(fedName: string | null | undefined): string {
  if (!fedName || fedName === "Sin FED asignado") {
    return "bg-yellow-500/10 text-yellow-700 dark:text-yellow-400 border-yellow-500/20"
  }

  // Assign unique colors to each FED
  const fedColors: Record<string, string> = {
    "Silvio Ridolfi": "bg-orange-500/10 text-orange-700 dark:text-orange-400 border-orange-500/20",
    "Jorge Pérez": "bg-red-500/10 text-red-700 dark:text-red-400 border-red-500/20",
    "Andrés Guzmán": "bg-blue-500/10 text-blue-700 dark:text-blue-400 border-blue-500/20",
    "Carlos Franco": "bg-green-500/10 text-green-700 dark:text-green-400 border-green-500/20",
    "Daniela Cortes": "bg-pink-500/10 text-pink-700 dark:text-pink-400 border-pink-500/20",
    "Macarena Duarte Buschiazzo": "bg-violet-500/10 text-violet-700 dark:text-violet-400 border-violet-500/20",
    "Marcos Pettiná": "bg-cyan-500/10 text-cyan-700 dark:text-cyan-400 border-cyan-500/20",
  }

  return fedColors[fedName] || "bg-gray-500/10 text-gray-700 dark:text-gray-400 border-gray-500/20"
}

/**
 * Returns a consistent color class for each educational level
 * Uses the primary level from combined levels (e.g., "Nivel Primario / Nivel Secundario" → Primario color)
 */
export function getNivelBadgeColor(nivel: string | null | undefined): string {
  if (!nivel) {
    return "bg-gray-500/10 text-gray-700 dark:text-gray-400 border-gray-500/20"
  }

  // Check for primary level keywords (order matters - check more specific first)
  if (nivel.includes("Nivel Inicial")) {
    return "bg-sky-500/10 text-sky-700 dark:text-sky-400 border-sky-500/20"
  }
  if (nivel.includes("Nivel Primario")) {
    return "bg-teal-500/10 text-teal-700 dark:text-teal-400 border-teal-500/20"
  }
  if (nivel.includes("Nivel Secundario")) {
    return "bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 border-indigo-500/20"
  }
  if (nivel.includes("Nivel Superior")) {
    return "bg-purple-500/10 text-purple-700 dark:text-purple-400 border-purple-500/20"
  }
  if (nivel.includes("Formación Profesional")) {
    return "bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/20"
  }
  if (nivel.includes("Formación Integral")) {
    return "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20"
  }
  if (nivel.includes("Educación Física")) {
    return "bg-lime-500/10 text-lime-700 dark:text-lime-400 border-lime-500/20"
  }
  if (nivel.includes("Psicología") || nivel.includes("C.E.C")) {
    return "bg-fuchsia-500/10 text-fuchsia-700 dark:text-fuchsia-400 border-fuchsia-500/20"
  }
  if (nivel.includes("NIVEL CENTRAL")) {
    return "bg-slate-500/10 text-slate-700 dark:text-slate-400 border-slate-500/20"
  }

  return "bg-gray-500/10 text-gray-700 dark:text-gray-400 border-gray-500/20"
}

/**
 * Formats FED name with prefix for display
 */
export function formatFedDisplay(fedName: string | null | undefined): string {
  if (!fedName) {
    return "FED: Sin asignar"
  }
  return `FED: ${fedName}`
}

/**
 * Splits combined plan values like "PNCE - PBA" into individual tokens ["PNCE", "PBA"].
 * Values without a separator (e.g. "Sin enlace", "ITINERANTE") are returned as a single token.
 */
export function parsePlanTokens(value: string | null | undefined): string[] {
  if (!value) {
    return []
  }
  return value
    .split("-")
    .map((token) => token.trim())
    .filter((token) => token.length > 0)
}

/**
 * Returns a consistent color class for a plan token (PNCE, PBA, or other statuses)
 */
export function getPlanTokenBadgeColor(token: string): string {
  const normalized = token.toUpperCase()
  if (normalized === "PNCE") {
    return "bg-[#417099]/10 text-[#417099] border-[#417099]/30"
  }
  if (normalized === "PBA") {
    return "bg-[#00AEC3]/10 text-[#00AEC3] border-[#00AEC3]/30"
  }
  if (normalized === "SIN ENLACE") {
    return "bg-slate-200 text-slate-600 dark:text-slate-300 border-slate-300"
  }
  return "bg-gray-500/10 text-gray-700 dark:text-gray-400 border-gray-500/20"
}

/**
 * Normaliza el campo de turnos para mostrar: saca comillas sueltas que
 * a veces vienen cargadas en el dato original (ej: '"DOBLE ESCOLARIDAD"')
 * y lo pasa a Title Case (ej: "MAÑANA, TARDE" -> "Mañana, Tarde").
 */
export function formatTurno(turnos: string | null | undefined): string | null {
  if (!turnos) return null
  const sinComillas = turnos.replace(/["']/g, "").trim()
  if (!sinComillas) return null

  return sinComillas
    .split(",")
    .map((parte) =>
      parte
        .trim()
        .toLowerCase()
        .split(" ")
        .map((palabra) => (palabra ? palabra[0].toUpperCase() + palabra.slice(1) : palabra))
        .join(" "),
    )
    .join(", ")
}
